...
# Experiment config
DEFAULT_SEEDS = [0, 42, 123, 456, 789, 2025, 2026, 2027, 2028, 2029]
...
def parse_seed_list(value: str) -> list[int]:
    """Parse comma-separated seeds from the command line."""
    try:
        seeds = [int(part.strip()) for part in value.split(",") if part.strip()]
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            "Seeds must be comma-separated integers, for example 0,42,123"
        ) from exc
    if not seeds:
...
def run_all_experiments(
    *,
    ...
    seed_values = DEFAULT_SEEDS if seeds is None else seeds
    ...
    # Count the workload.
    baseline_count = len(BASELINE_ALGORITHMS) * len(SCENES) * len(seed_values)
    ppo_count = len(ALPHA_VALUES) * len(SCENES) * len(seed_values)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "逗号分隔的随机种子列表，例如 "
            "0,42,123,456,789,2025,2026,2027,2028,2029"
        ),
    )

...
def plot_with_error_bars(
    ...
    output_dir: Path,
    *,
    baseline_income_path: Path,
    ppo_income_path: Path,
    alpha_value: float,
) -> None:
    ...
    if 'alpha' in primary_df.columns:
        primary_df = primary_df[
            ...
            (primary_df['alpha'].astype(float).round(1) == round(alpha_value, 1))
        ]
...
def plot_income_cdf(
    ...
    *,
    ...
    """Plot one shock-scene driver-income sample for each main policy."""

    if not baseline_income_path.exists() or not ppo_income_path.exists():
        print(
            "  警告: 缺少 shock 收入快照，跳过 shock_income_cdf.png。"
            "请先运行 evaluate_baselines.py 和 train_ppo.py。"
        )
        return

    baseline_df = pd.read_csv(baseline_income_path)
    ppo_df = pd.read_csv(ppo_income_path)

    if "scene" in ppo_df.columns:
        ppo_df = ppo_df[ppo_df["scene"] == "shock"]
    if "alpha" in ppo_df.columns:
        ppo_df = ppo_df[ppo_df["alpha"].astype(float).round(6) == round(alpha_value, 6)]

    grouped: dict[str, list[float]] = {}
    for algorithm in ["Local-First", "Demand-Greedy"]:
        values = baseline_df[baseline_df["algorithm"] == algorithm].sort_values(
            "driver_index"
        )["income"]
        if not values.empty:
            grouped[algorithm] = values.astype(float).tolist()

    if not ppo_df.empty:
        grouped[f"PPO (alpha={alpha_value:g})"] = (
            ppo_df.sort_values("driver_index")["income"].astype(float).tolist()
        )

    if not grouped:
        print("  警告: 收入快照为空，跳过 shock_income_cdf.png")
        return
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "shock_income_cdf.png", dpi=300)
    plt.close()
    print("  ✓ 已生成: shock_income_cdf.png")
...
def main() -> None:
    ...
    try:
        ...
        plot_with_error_bars(
            summary_df,
            args.figures_dir,
            baseline_income_path=args.baseline_incomes,
            ppo_income_path=args.ppo_incomes,
